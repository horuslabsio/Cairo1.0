...
In this section we will how to declare and deploy starknet smart contract, but before that we will have to understand some concepts such as:
...
To check learn more about UDC, check out the links below:
...
Declaring a contract registers a `blueprint` of a smart contract on-chain usinf its Sierra code. The blueprint is also known as the contract class and it's identified by its `class hash`.
...
However you chose to do it, ensure that the funding transaction reaches the "PENDING" status before moving on. You can look for it on [Starkscan](https://starkscan.co/) or [Voyager](https://goerli.voyager.online/).
